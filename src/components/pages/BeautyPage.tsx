...
export const BeautyPage: React.FC<BeautyPageProps> = ({ onBack }) => {
  const [userImage, setUserImage] = useState<string | null>(null);
  const [productImage, setProductImage] = useState<string | null>(null);
  const [showCameraModal, setShowCameraModal] = useState(false);
  const [cameraFor, setCameraFor] = useState<'user' | 'product'>('user');
  ...
  const handleFileSelect = (file: File, type: 'user' | 'product') => {
  ...
  const handleTryOn = async () => {
    if (!userImage || !productImage) {
      toast({
        ...
        description: "Please upload both your photo and the beauty product",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Beauty try-on complete!",
        description: "Here's how the makeup looks on you",
      });
    } catch (error) {
      toast({
        title: "Try-on failed",
      ...
      <div className="max-w-md mx-auto">
        ...
        <GlassCard variant="elevated" className="mb-6">
          ...
          <GlassCardContent className="p-6">
            <p className="text-muted-foreground mb-6">
              Upload your photo and a beauty product to see how it looks on you
            </p>
            ...
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-3">Beauty Product</h3>
              <div className="border-2 border-dashed border-muted rounded-lg p-4">
                {productImage ? (
                  <img src={productImage} alt="Product" className="w-full h-32 object-cover rounded-lg" />
                ) : (
                  <div className="text-center text-muted-foreground">
                    <Upload className="w-8 h-8 mx-auto mb-2" />
                    <p className="text-sm">Upload makeup/skincare product</p>
                  </div>
                )}
                <div className="flex space-x-2 mt-3">
                  <GlassButton 
        ...
        {result && (
          <GlassCard variant="elevated">
            <GlassCardHeader>
              <GlassCardTitle>Your Beauty Try-On</GlassCardTitle>
            </GlassCardHeader>
            <GlassCardContent className="p-6">
